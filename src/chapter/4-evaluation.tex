...
\begin{description}
    ...
    \item[\ac{ha}6] erwartet, dass die zuvor erstellten Oberflächendesigns mit \acp{fxml}\footnote{
        Beschreibungssprache für Oberflächen im JavaFX-Framework, basierend auf \ac{xml}.
...
\begin{itemize}
    ...
    Dies war nur möglich, wenn bestimmte Wörter in der Task-Beschreibung in Backticks (\verb|``|) gesetzt wurden.
    Es entstand zusätzlicher Aufwand, der jedoch nicht immer eine sinnvolle Filterung ermöglichte.
    ...
    \item Der Ablauf erlaubte nicht die Auswahl mehrerer Codeabschnitte als Teil einer Bewertung in einem Schritt.
    ...
    Damit verbundene Probleme haben sich besonders in \ac{ha}3 von \ac{pm} manifestiert\footnote{
        \url{https://github.com/fujaba/fulib.org/issues/196\#issuecomment-981629981}
    }.
\end{itemize}
...
Bei groben Fehlern führte dies dazu, dass vermeintlich bewertete Teilaufgaben nochmal überprüft werden mussten, wodurch zusätzlicher Zeitaufwand entstand.
...
Hauptursache war die Auswahl von Codeabschnitten, welche nicht eindeutig waren und unzureichenden Kontext enthielten.
...
Dies erwies sich als nützlich genug, um die Benutzungsfehler zu reduzieren\footnote{
    \url{https://github.com/fujaba/fulib.org/issues/196\#issuecomment-1025851695}
}.
...
\begin{table}
    \centering
    \caption{Punkte-Metriken in \ac{pm}}
...
Die Anzahl der Teilaufgaben zur Bewertung orientiert sich an der Punktzahl und ist stets kleiner oder gleich.
...
\begin{table}
    \centering
    \caption{Abgaben-Metriken in \ac{pm}}
...
\begin{table}
    \centering
    \caption{Bewertungen und Effektivität in \ac{pm}}
...
Die spezielle Kategorie der automatisch erstellten Bewertungen, die später \textbf{bearbeitet} wurden, ist zur Vollständigkeit aufgeführt.
...
Weiterhin wurden dort Tests und Implementierung nach dem Test-First-Prinzip entwickelt, welche hohen Varianzen des entstandenen Codes erlaubten.
...
Der Grund dafür ist die Vielzahl von Teilaufgaben, bei denen das Datenmodell mit fulib generiert wurde.
...
Ein Teil der Punkte dafür war in zwei Teilaufgaben angesiedelt, die hauptsächlich manuell bewertet wurden.
Die verbleibenden Punkte konnten aufgrund von strikten Vorgaben ebenfalls automatisch bewertet werden.

\subsubsection{Effizienz}

Die Effizienz bezeichnet die Zeit, die durch die Verwendung von Code Search eingespart werden kann.
...
Zunächst ist die durchschnittliche \textbf{Zeit pro Bewertung} einer Teilaufgabe angegeben, die hier zwischen 7 und 10 Sekunden liegt.
Das deutet darauf hin, dass es sich bei der Bewertung einzelner Teilaufgaben generell nur um einen kurzen Arbeitsschritt handelt, der häufig wiederholt wird.
...
\begin{table}
    \centering
    \caption{Effizienz in \ac{pm}}
    \begin{tabular}{|l|l|l|l|l|}
    \hline
        \acs{ha}  & Zeit/Bewertung & Manuell & Code Search & Effizienz \\ \hline
...
\begin{table}
    \centering
    \caption{Bewertungszeit (Minuten) einzelner Lösungen aus \ac{pm}}
    ...
    \hline
        ...
        11 & 14 & 9  & 13 & 7  & 2  & 10 & 5 & 7  & 3 & 2  & ~  & ~  & ~  & ~  & ~  & ~  & ~  & ~  & 75    \\ \hline
    \end{tabular}
    \label{tbl:pm-overhead}
\end{table}

Tabelle~\ref{tbl:pm-adapted-efficiency} wiederholt die \textbf{Gesamtzeit} aus Tabelle~\ref{tbl:pm-overhead} und gibt an, wie viel Zeit die Person im Bewertungsfenster verbracht hat (\textbf{Bewertet}).
Die Division beider Werte ergibt jeweils den \textbf{Faktor}, der eine Hochrechnung auf alle Bewertenden ermöglicht.
Mithilfe der Formel~\ref{eqn:adapted-efficiency} kann schließlich die \textbf{angepasste Effizienz} berechnet werden.
Sie repräsentiert die Effizienz von Code Search unter Beachtung des zusätzlichen Zeitaufwands.
Die Werte $t_{Code Search}$ und $t_{Manuell}$ entstammen der Tabelle~\ref{tbl:pm-efficiency}.

\begin{table}
    \centering
    \caption{Angepasste Effizienz in \ac{pm}}
    \begin{tabular}{|l|l|l|l|l|}
    \hline
        \acs{ha} & Gesamtzeit  & Bewertet  & Faktor  & Angepasste Effizienz  \\ \hline
        7  & 212 min & 53:07 min & 4.0 & 23\% \\ \hline
        8  & 151 min & 23:54 min & 6.3 & 23\% \\ \hline
        9  &  31 min & 14:16 min & 2.2 & 65\% \\ \hline
        10 &  77 min & 19:27 min & 4.0 & 23\% \\ \hline
        11 &  72 min & 25:40 min & 2.8 & 39\% \\ \hline
    \end{tabular}
...
In diesem Abschnitt werden einige Möglichkeiten zur Optimierung beschrieben, die in der Veranstaltung ausprobiert wurden.
...
\begin{description}
    ...
    Sie kann sich abhängig von den Einstellungen der \ac{ide}, mit der die Lösung verfasst und formatiert wurde, unterscheiden.
    \item[Kommentare.]
    ...
    Einfache \code{int}-Variablen werden oft mit Buchstaben wie \code{i}, \code{j}, \code{x} oder \code{n} benannt.
    ...
    Objekte folgen meist dem gleichen Muster der Benennung nach ihrer Rolle, seltener und in eindeutigen Fällen nach ihrem Typ\footnote{
...
Dies wäre mit Code Search nicht prüfbar, wenn die Kommentare wie Leerzeichen behandelt werden.
...
Sonstige Bezeichner, beispielsweise \acp{id} von Oberflächenelementen, wurden in den \acp{ha} 4, 6 und 9 vorgegeben.
...
Statt GitHub Classroom wird ein eigenes Abgabesystem verwendet, in dem Studierende ihre Lösungen als Archivdateien verpackt hochladen.
...
Aus diesem Grund wird nachfolgend nur die Effektivität von Code Search ermittelt.
...
\begin{table}
    \centering
    \caption{Teilaufgaben und Lösungen in \ac{algods}}
...
\begin{table}
    \centering
    \caption{Bewertungen und Effektivität in \ac{algods}}
    \begin{tabular}{|l|l|l|l|l|l|l|l|}
    \hline
        ~ & \multicolumn{5}{c|}{Bewertungen} & ~ & ~ \\
        \acs{ha} & Manuell & Code Search & Bearbeitet & Übersprungen & Gesamt & Effektivität & Link \\ \hline
...
Bewertungen sind aufgeteilt in die Kategorien \textbf{Manuell}, \textbf{Code Search}, \textbf{Bearbeitet} (von Code Search erstellt und händisch angepasst) und \textbf{Übersprungen} (berechnet aus übersprungenen Teilaufgaben mal betrachtete Abgaben).
...
Bei \ac{ha}1 und \ac{ha}6 sind mit 42\% und 63\% ähnlich gute Ergebnisse der Effektivität zu verzeichnen wie in verschiedenen \ac{pm}-Hausaufgaben.
