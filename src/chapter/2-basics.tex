...
Zu Beginn der zweiten und dritten Woche ist sichtbar, dass Bewertung und Konzeption gleichzeitig stattfinden.
Folglich ist es sinnvoll, dass unterschiedlichen Personen diese beiden Phasen betreuen.

\subsubsection{Konzeption}

Die Konzeptionsphase beinhaltet das Verfassen der Aufgabenstellung, die Berechnung und Vergabe der Punktzahlen und damit einhergehende Bewertungsrichtlinien, sowie das bedingte Erstellen einer Musterlösung.
...
\begin{description}
    ...
    Dies ist teilweise durch Plugins möglich, die von den Sprachautor:innen bereitgestellt werden, aber mitunter signifikanten Aufwand benötigen.
    ...
    Dies kann die Verbreitung von Sprachen bei begrenztem Entwicklungsbudget einschränken.
    ...
    Abhängig von der Architektur des Editors ist dies jedoch auch ohne ein solches Protokoll möglich\footnote{
        Beispielsweise in IntelliJ, das Vorgaben für Multithreading macht um schreibende Aktionen (Texteingabe, \ldots) von lesenden Aufgaben (Syntaxanalyse, Diagnostics, Highlighting, \ldots) zu trennen.
        Siehe \url{https://plugins.jetbrains.com/docs/intellij/general-threading-rules.html}.
    }.
\end{description}
...
\begin{description}
    ...
    Es ist hilfreich, während des Schreibens von Code mögliche Syntaxfehler oder andere Probleme rot oder gelb unterstrichen hervorzuheben.
    Ein Language Server kann dies mittels Diagnostics implementieren.
    Im Gegensatz zu anderen Funktionen werden diese asynchron vom Client \ac{bzw} Editor angefragt.
    ...
    Beispielsweise können dadurch einfache Refactorings wie das Extrahieren einer Methode implementiert werden.
    % TODO Geht schon mit Commands, aber wirklich notwendig hier?
    % Nicht möglich ist das Erfragen von weiterem Input des Benutzenden.
...
Durch Diagnostics können beispielsweise Rechtschreibprüfung oder Linter\footnote{Programme, die Quellcode anhand von verschiedener Analyseverfahren auf häufige Fehlerquellen untersuchen} umgesetzt werden.
...
Dabei werden standardmäßig Leerzeichen und Zeichen wie Punkte, Kommata oder Anführungsstriche verworfen~\cite{elastic-analysis-analyzers}.
...
Sie bietet gegenüber JavaScript durch die Typisierung eine höhere Sicherheit gegen Programmierfehler.
...
Seine Zuständigkeiten umfassen eine komponentenbasierte View, Dependency Injection für Services und die Aufteilung von logischen Programmteilen in Module.
Angular abstrahiert das Rendering von Komponenten zu \ac{html} für die Darstellung im Browser.
Services erlauben die Trennung von Oberflächenlogik, Business Logic und Datenzugriff.
...
Es definiert semantische Farben sowie Aussehen, Anordnung und Abstände von Buttons, Formelementen und sonstigen Komponenten.
...
Dazu gehören die von Angular bekannte Aufteilung in Module und Services, die zugehörige Dependency Injection, sowie darüber hinaus Controller, welche die \ac{http}-Schnittstellen definieren.
...
Im Gegensatz zur dokumentierten\cite{vsc-language-server-guide} Architektur kann dadurch bessere Trennung von Zuständigkeiten und eine erweiterbare Grundstruktur geschaffen werden.
