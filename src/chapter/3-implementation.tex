...
Dadurch ist mit einer höheren Abgabenanzahl und -vielfalt bei gleichzeitig geringerer Schwierigkeit und Komplexität im Vergleich zu späteren Aufgaben zu rechnen.
...
Eine Ansprechperson, beispielsweise die Übungsleitung, und deren Email-Adresse werden in den Feldern \textbf{Contact Name} und \textbf{Contact Email} festgelegt.
...
Aus dem Repository-Namen kann der GitHub-Anmeldename des Studierenden ermittelt werden, der für die Zuordnung der Lösung verwendet wird.
...
Vergleichsweise große Dateien (> \SI{64}{\kibi\byte}\footnote{
    Die Suche nach Quelltext-Dateien größer als \SI{64}{\kibi\byte} in der GitHub-Organisation "sekassel" ergab lediglich Ergebnisse aus Node.js-Projekten, darunter primär \code{package-lock.json}-Dateien und Dateien in versehentlich hochgeladenen \code{node_modules}-Ordnern.
...
In dem modalen Formular kann die Beschreibung (\textbf{Description}) und die erreichbare Punktzahl (\textbf{Points}) eingestellt werden.
In der ersten Abbildung ist zu sehen, dass die Existenz von Unteraufgaben die automatische Berechnung der summierten Punktzahl ermöglicht, weshalb der Zauberstab-Button neben dem Eingabefeld sichtbar ist.
...
Wenn eine Zeile das Format nicht befolgt, wird sie rot markiert, um den Benutzenden auf das Problem hinzuweisen.
...
Diese Richtlinien haben ihre Ursprünge aus der Verwendung von GitHub, da Issues, Pull Requests und Kommentare auf dieser Plattform in Markdown verfasst werden.
...
Die Punktzahl ist zunächst mit "\texttt{?/22}" als unausgewertet markiert.
...
Die Bewertung in diesem Fenster ist zunächst einfach gehalten und für schnelle Bedienbarkeit optimiert.
...
Diese enthält Metadaten wie das Bewertungsdatum und den zugrundeliegenden Commit.
...
Einerseits kann das Formular zum Anlegen eines Issues auf GitHub mit \textbf{Draft Issue} geöffnet werden, sodass der Text ein weiteres Mal überprüft oder angepasst werden kann.
...
Evaluations und Time Tracking können die Ansicht verändern, um Punktzahlen statt Bewertungen von Teilaufgaben als Berechnungsgrundlage zu verwenden.
...
Studierende müssen statt Assignment-Token eine Solution mit einer \textbf{\acs{id}} und zugehörigem \textbf{Token} angeben.
Diese können der Fußzeile des Feedbacks entnommen werden.
Der \textbf{\acs{api} Server} kann für Entwicklungszwecke geändert werden.

Um die Einrichtung zu beschleunigen, müssen die Einstellungen \ac{idR} nicht von Hand getätigt werden.
...
Andernfalls können der GitHub-Name und der relative Pfad nicht bestimmt werden.
...
Die wichtigste Funktion von fulibFeedback für Bewertende ist die Auswahl\footnote{
    Bezeichnet die Aktion, die mit Maus oder Tastatur in gewöhnlichen Texteditoren durchgeführt wird, um beispielsweise Text zum Kopieren zu markieren.
} von Codeabschnitten.
fulibFeedback erkennt dies und sendet den ausgewählten Code zusammen mit Kontextdaten wie Assignment, Solution, Datei sowie Zeilen- und Spaltennummern an den Backend-Dienst von fulib.org.
...
In jeder Abgabe können \ac{uU} mehrere Dateien gefunden werden;
die Gesamtanzahl über alle Abgaben ist in \textbf{Files} summiert.
...
Eine Toast-Nachricht weist abschließend darauf hin, wie viele weitere Abgaben tatsächlich betroffen sind\footnote{
    Dies kann sich von der Vorschau unterscheiden, wenn andere Abgaben bereits eine Bewertung für die Teilaufgaben enthalten.
}.
...
Im Test musste unter anderem ein Datenmodell initialisiert werden.
...
Die Einstellungen werden in einer Paketdatei anhand von Typ, Auswahlmöglichkeiten, Beschreibung, regulären Ausdrücken für die Validierung und zugehörigen Fehlermeldungen angegeben.
Darauf basierend werden automatisch die Eingabefelder in den Einstellungen von \ac{vsc} gezeigt, wie in Abbildung~\ref{fig:fulibFeedback-settings} zu sehen war.
...
Das Framework erlaubte eine bessere Strukturierung durch verschiedene Services für Sprachfeatures.
...
Es handelt sich dabei nicht direkt um die intendierte Verwendung dieses Sprachfeatures, da es nicht für die generelle Erkennung der Auswahl, sondern zur Bereitstellung von Aktionen vorgesehen ist.
